					...
					public void run() {
						try {
							...
							while(!FIM && FLAG_BUSCA) {  //enquanto fim nao for verdadeiro e nÃ£o tiver apertado o btn parar busca
								...
								if(Ret_busca[0].indexOf(textField_Nome.getText().toUpperCase())>0 || textField_Nome.getText().isEmpty()) { //verifica se retornou o nome que tem no campo da busca	
									if(Ret_busca[0] != "FIM") Codigo.consulta_to_Html(); //para não gerar o link da pagina FIM
								}	
								Codigo.n_page++;								
